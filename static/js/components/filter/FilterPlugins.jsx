...
const FilterPlugins = ({ group }) => {
  ...
  const handleChangeUpdateAnnotations = async (event) => {
    ...
    const result = await dispatch(
      filterActions.editUpdateAnnotations({
  ...
  const handleChangeAutoFollowupComment = async () => {
    ...
    // }
    // const result = await dispatch(
    //   filterActions.editAutoFollowup({
    //     filter_id: filter.id,
  ...
  const handleChangeAutoFollowup = async (event) => {
    ...
    // if there is no autoFollowUp object, create one
    if (filter_v.altdata.autoFollowUp) {
      newAutoFollowup = { ...filter_v.altdata.autoFollowUp, active: target };
    }
    const result = await dispatch(
      filterActions.editAutoFollowup({
        filter_id: filter_v.id,
        autoFollowup: newAutoFollowup,
      }),
    );
    if (result.status === "success") {
      dispatch(showNotification(`Set autoFollowup to ${target}`));
    }
    dispatch(filterActions.fetchFilterVersion(fid));
  };
  ...
  useEffect(() => {
    ...
    if (filter_v?.autoFollowup?.pipeline) {
      setValue("pipeline_auto_followup", filter_v.autoFollowup.pipeline);
    }
    if (filter_v?.autoSave?.comment) {
      setAutosaveComment(filter_v.autoSave.comment);
    }
    if (filter_v?.autoFollowup?.comment) {
      setAutoFollowupComment(filter_v.autoFollowup.comment);
    }
  }, [filter_v]);
  ...
  const onSubmitSaveAutosaveSaver = async (e) => {
    ...
    if (result.status === "success") {
      if (e.target.value === "unassigned") {
        dispatch(
          showNotification(
            `Unassigned autosave saver, will use boom's default saver`,
          ),
        );
  ...
  useEffect(() => {
    // not using API/boom_filter duck here as that would throw an error if filter does not exist on K
    const fetchInit = {
    ...
    const fetchFilterVersion = async () => {
      const response = await fetch(`/api/filters/${fid}`, fetchInit);
      let json = "";
      try {
      ...
      if (json.status === "success") {
        await dispatch(filterActions.fetchFilterVersion(fid));
      }
    };
                  ...
                  <div style={{ display: "flex", flexDirection: "row" }}>
                    {filter_v?.fv && (
                      <FormControlLabel
                        ...
                        control={
                          <Switch
                            checked={filter_v.altdata.autoAnnotate}
                            size="small"
                    ...
                    {filter_v?.fv && (
                      <FormControlLabel
                        ...
                        control={
                          <Switch
                            checked={
                              filter_v.altdata.autoSave === true ||
                              filter_v.altdata.autoSave?.active === true
                            }
                            size="small"
                      ...
                      (filter_v.altdata.autoSave === true ||
                        ...
                        <>
                          <TextField
                            ...
                            rows={1}
                            defaultValue={filter_v.altdata.autoSave?.comment}
                            onChange={(event) =>
                      ...
                      ((group?.group_users || []).filter(
                        ...
                        <div>
                          <InputLabel id="saverSelectLabel">
                            Group user to use as the saver (optional, group
                            admin only)
                          </InputLabel>
                          <Select
                            inputProps={{
                          ...
                          >
                            ...
                            {(group?.group_users || [])
                              .filter(
                                (group_user) => group_user.can_save === true,
                              )
                              .map((group_user) => (
                                <MenuItem
                    ...
                    {filter_v?.fv && (
                      <FormControlLabel
                        ...
                        control={
                          <Switch
                            checked={
                              filter_v?.altdata.autoFollowup?.active === true
                            }
                            size="small"
                          ...
                          >
                            ...
                            <form
                              onSubmit={handleSubmit(
                                onSubmitSaveAutoFollowupFilter,
                              )}
                            >
                              <DialogContent>
                                <DialogContentText>
                                  boom filter definition. For a detailed
                      ...
                      filter_v.altdata.autoFollowup?.active === true && (
                        <>
                          <TextField
                            ...
                            defaultValue={
                              filter_v.altdata.autoFollowup?.comment
                            }
                            onChange={(event) =>
                    ...
                    {filter_v?.fv &&
                      filter_v?.altdata.autoFollowup?.active === true && (
                        <>
                          ...
                          {filter_v?.altdata.autoFollowup?.allocation_id &&
                              ...
                              <>
                                ...
                                >
                                  ...
                                  <DialogContent>
                                    ...
                                    >
                                      {JSON.stringify(
                                        filter_v.altdata.autoFollowup.payload,
                                        null,
                                        2,
                                      )}
                                    </Typography>
                                    ...
                                    <Form
                                      schema={
                                        selectedAllocationParams.formSchema
                                      }
                                      validator={validator}
                                      uiSchema={
                                        selectedAllocationParams.uiSchema
                                      }
                                      liveValidate
                      ...
                      filter_v?.altdata.autoFollowup?.active === true && (
                        <div>
                          <InputLabel id="targetGroupsSelectLabel">
                            Target groups (optional, already includes the group
                            of the filter)
                          </InputLabel>
                          <Select
                            inputProps={{
                        ...
                        <>
                          ...
                          <div style={{ marginTop: "1rem" }}>
                            ...
                            <TextField
                              ...
                              defaultValue={
                                filter_v.altdata.autoFollowup?.radius
                              }
                              onChange={(event) =>
                          ...
                          <div style={{ marginTop: "1rem" }}>
                            <InputLabel id="auto_followup_constraints_tns_age">
                              Cancel if TNS reported in the last N hours. Leave
                              empty to disable
                            </InputLabel>
                            <TextField
                          ...
                          >
                            {filter_v?.fv &&
                              filter_v?.altdata.autoFollowup?.active ===
                                true && (
                                <div>
                                  <InputLabel id="ignoreAllocationSelectLabel">
                                    Cancel if existing pending/completed
                                    triggers with:
                                  </InputLabel>
                                  <Select
                                    inputProps={{
                                    ...
                                    multiple
                                  >
                                    {allocationListApiClassname?.map(
                                      (allocation) => (
                                        <MenuItem
                                          value={allocation.id}
                                          key={allocation.id}
                                        ...
                                        >
                                          {`${
                                            instLookUp[allocation.instrument_id]
                                              ?.name
                                          } - ${
                                            groupLookUp[allocation.group_id]
                                              ?.name
                                          } (PI ${allocation.pi})`}
                                        </MenuItem>
                                      ),
